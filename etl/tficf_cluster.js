...
var outTopDir = 'tficf/104/job';

var basename_keyword = 'input/keywords_merge.txt';
var basename_jobgroup = 'input/jobgroup.txt';
var basename_tf_idf = 'tf_icf.txt'; //	tf+icf
var basename_tfidf = 'tficf.txt'; //	tf*icf
...
function walkJobCat(dir, outDir, done) { //	per job category
	...
	fs.readdir(dir, function (err, list) {
		...
		q.drain = function () {
			...
			//console.log(matrix);
			var outFile2 = path.join(groupDir, basename_tfidf);
			...
			var outFile3 = path.join(groupDir, basename_tf_idf);
...
function walkDaily(dir, outDir, done) { //	per day
	...
	fs.readdir(dir, function (err, list) {
		...
		async.eachSeries(list.sort(), function (baseFolder, callback) {
			...
			var folder = path.join(dir, baseFolder + '/' + group_mode.name);
...
function readKeywords() {
	...
	async.parallel({
		...
		emitter.emit('log', NEWLINE + 'Totally ' + results.jobgroup + ' job/group counted.');
		emitter.emit('log', NEWLINE + 'Totally ' + count + ' keywords counted.');
		//process.exit();
		emitter.emit('keyword', NEWLINE);
	});
}
