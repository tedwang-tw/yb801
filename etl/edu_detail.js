...
var inTopDir = '../crawler/raw/104/edu';
var outTopDir = 'text/104/edu';

var basename_err = 'error.txt';
var basename_status = 'status.txt';
var filename_status;

var filename_preset = path.join(__dirname, 'edu_detail_cfg.json');
var presetList;
...
function scrapeCourse(dir, outDir, task, done) {
...
function walkJobCat(dir, outDir, done) { //	per job category
	...
	fs.readdir(dir, function (err, list) {
		...
		var q = async.queue(function (task, taskCB) {
				scrapeCourse(dir, outDir, task, function (err, count) {
					itemCounter += count;
					setImmediate(taskCB);
				});
			}, CONCURRENCY);
		...
		q.drain = function () {
			emitter.emit('doneJobCat', NEWLINE + 'Totally ' + itemCounter + '/' + listLen + ' courses/files processed.');
