...
function defaultCrawler(options, env, callback) {
	var fileName_s = path.join(options.outDir, options.codes.comCode + '_' + options.codes.jobCode + env.rawExt);
	...
	setImmediate(function () {
		...
		superagent
		.get(env.baseUrl + options.postfix)
...
function startCrawler(env, dateStr, outerCallback) {
	...
	var fileName = path.join(outDirDate, 'joburl_rec.txt');
	if (fs.existsSync(fileName))
		fs.unlinkSync(fileName);

	async.mapLimit(env.codeList, env.concurrency, function (codes, callback) {
		var options = {
			postfix : codes.link,
			outDir : outDirDate,
			codes : codes,
